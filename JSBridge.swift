...
class JSBridge: NSObject, WKScriptMessageHandler {
    ...
    private func callJSCallback(_ callback: String?, result: Any) {
        ...
        if let array = result as? [[String: Any]],
            ...
            // Caso especial para getResults: armazena o array e retorna
            executeJS("window._h5gg_results = \(jsonString);")
            return
        }
        
        // Para todos os outros casos (números, booleanos, strings, dicionários)
        if let jsonData = try? JSONSerialization.data(withJSONObject: result),
           let jsonString = String(data: jsonData, encoding: .utf8) {
            resultString = jsonString
        } else {
            // Fallback para tipos simples (Int, Bool)
            resultString = "\(result)"
        }
